...
class HierarchicalPlanner:
    ...
    def plan_with_mcts(
        ...
        """Create a plan using MCTS for exploration.

        Uses Monte Carlo Tree Search to explore different decomposition strategies
        ...
        Returns:
            TaskGraph with optimized task decomposition
        """
        ...
        # Define decomposition function for MCTS
        def decompose_fn(task_desc: str) -> list[str]:
            """Decompose task into subtasks."""
            # Create temporary task
            temp_task = Task(title=task_desc, description=task_desc)
            decomposition = self._decompose_heuristic(temp_task, context, depth=0)
            return [st.title for st in decomposition.subtasks]

        # Define evaluation function for MCTS
        def evaluate_fn(path: list[str]) -> float:
            """Evaluate quality of a task decomposition path."""
            # Prefer shorter paths (simpler plans)
            length_penalty = 1.0 / (1.0 + len(path) * 0.1)

            # Prefer paths with verification steps
            has_verification = any(
                "test" in task.lower() or "verify" in task.lower() or "check" in task.lower()
                for task in path
            )
            verification_bonus = 0.3 if has_verification else 0.0

            # Prefer paths with clear dependencies
            has_dependencies = len(path) > 2
            dependency_bonus = 0.2 if has_dependencies else 0.0

            return length_penalty + verification_bonus + dependency_bonus

        # Run MCTS to find optimal decomposition
        optimal_subtasks = mcts.plan(goal, decompose_fn, evaluate_fn)

        # Build TaskGraph from optimal plan
        graph = TaskGraph()

        # Create root task
        root_task = Task(
            title=goal,
            description=f"Root goal (MCTS-optimized): {goal}",
            priority=TaskPriority.CRITICAL,
            metadata={"level": 0, "is_root": True, "mcts_optimized": True},
        )
        graph.add_task(root_task)

        # Add subtasks from MCTS result
        prev_task_id = None
        for i, subtask_title in enumerate(optimal_subtasks):
            subtask = Task(
                title=subtask_title,
                description=f"Subtask {i+1}: {subtask_title}",
                priority=TaskPriority.HIGH if i == 0 else TaskPriority.NORMAL,
                metadata={"level": 1, "parent_id": root_task.id, "mcts_order": i},
            )
            graph.add_task(subtask)

            # Link to root
            graph.add_dependency(root_task.id, subtask.id)

            # Sequential dependency (each task depends on previous)
            if prev_task_id:
                graph.add_dependency(subtask.id, prev_task_id)
