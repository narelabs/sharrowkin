"""In-process tool runner for Sharrowkin Agent.

Adapted from Google Antigravity SDK.

Tools are Python callables that run directly in the agent process. The
...
automatically. Schema generation strips injectable parameters so the
...
from core.tool_system import tool_context as tool_context_module
