...
def summarize_workspace(summaries: list[FileSummary], max_files: int = 60) -> str:
    ...
    for summary in summaries[:max_files]:
        dir_name = str(Path(summary.path).parent) if '/' in summary.path or '\\' in summary.path else '.'
    ...
    for dir_name in sorted(by_dir.keys()):
        lines.append(f"\n📁 {dir_name}/")
        for summary in by_dir[dir_name]:
            file_name = Path(summary.path).name
            lines.append(f"  📄 {file_name} [{summary.language}, {summary.line_count} lines]")

            if summary.error:
                lines.append(f"    ⚠️ error: {summary.error}")
                continue

            # Show imports with context
            if summary.imports:
                imports = ", ".join(sorted(set(summary.imports))[:8])
                lines.append(f"    📦 imports: {imports}")

            # Show symbols with more detail
            if summary.symbols:
                lines.append(f"    🔧 symbols:")
                for symbol in summary.symbols[:12]:  # Increased from 8 to 12
    ...
    if len(summaries) > max_files:
        lines.append(f"\n... {len(summaries) - max_files} more files omitted")

    lines.append("\n=== END WORKSPACE STRUCTURE ===")
    return "\n".join(lines)
