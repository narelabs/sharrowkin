    ...
    def learn_failure(
        ...
        *,
        ...
        Args:
            ...
            error_type: Type of error (syntax, logic, test, etc.)
            error_message: Error message
            attempted_approach: What approach was tried
            iteration: Which iteration failed
        """
        if not self.enabled or self.dsm is None:
            return

        # Store failure in DSM with negative importance
        failure_content = f"""Task: {task}

Status: FAILED (iteration {iteration})
...
Lesson: This approach did not work. Try alternative strategy."""

        self.dsm.update_from_interaction(
            f"FAILURE: {task}",
            failure_content,
            importance=0.3,  # Lower importance than success
            metadata={
                "source": "sharrowkin_failure",
